...
def main():
    ...
    if args.shift:
        if args.soft_shift:
            full_arch_name += '_softshift{}_{}'.format(args.shift_div, args.shift_place)
        else:
    ...
    model = TSN(num_class, args.num_segments, args.modality,
                ...
                is_shift=args.shift, shift_div=args.shift_div, shift_place=args.shift_place, is_softshift=args.soft_shift, shift_init_mode=args.shift_init_mode,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if 1:
            if args.lr_type != 'ReduceLROnPlateau':
                ...
                prec1 = validate(val_loader, model, criterion, epoch, log_training, tf_writer, scheduler=scheduler)
    ...
    output_time = 'Totally cost %.3f h: %.3f min: %.3f s' %(time_cost/60/60, time_cost/60, time_cost%60)
    print(output_time)
...
def validate(val_loader, model, criterion, epoch, log=None, tf_writer=None, scheduler=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (input, target, path) in enumerate(val_loader):
            ...
            each_class_output_buffer = torch.cat((each_class_output_buffer, output.detach().cpu().float()), 0)
            each_class_target_buffer = torch.cat((each_class_target_buffer, target.detach().cpu().float()), 0)
...
def calculate_accuracy_each_class(outputs, targets, n_classes):
    _, pred = outputs.topk(1, 1, True)
    pred = pred.t()
    confusion_matrix = torch.zeros(n_classes, n_classes)
    try:
        for t, p in zip(targets.t(), pred.squeeze()):
