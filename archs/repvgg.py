...
#   May be useful for quantization or pruning.
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        if isinstance(branch, nn.Sequential):
            ...
            running_mean = branch.bn.running_mean # accumulated mean
            running_var = branch.bn.running_var # accumulated standard deviation
            gamma = branch.bn.weight # accumulated learned scaling factor 
            beta = branch.bn.bias # accumulated learned bias
            eps = branch.bn.eps
        else:
...
def repvgg_model_convert(model:torch.nn.Module, build_func, save_path=None):
    converted_weights = {}
    print("Re-paraming training-time model weights ...")
    for name, module in model.named_modules():
        if hasattr(module, 'repvgg_convert'):
            kernel, bias = module.repvgg_convert() # 合并conv和bn
...
def repvgg_A0(pretrained=True, deploy=True):
    model = create_RepVGG_A0(deploy=deploy)
    if pretrained:
        if deploy:
            checkpoint_pth = "/home/chenkangyang/datasets/model_pretrained/RepVGG/RepVGG-A0-deploy.pth"
        else:
            checkpoint_pth = "/home/chenkangyang/datasets/model_pretrained/RepVGG/RepVGG-A0-train.pth"
...
def repvgg_B1g2(pretrained=True, deploy=True):
    model = create_RepVGG_B1g2(deploy=deploy)
    if pretrained:
        if deploy:
            checkpoint_pth = "/home/chenkangyang/datasets/model_pretrained/RepVGG/RepVGG-B1g2-deploy.pth"
        else:
            checkpoint_pth = "/home/chenkangyang/datasets/model_pretrained/RepVGG/RepVGG-B1g2-train.pth"
...
if __name__ == "__main__":
    pretrain = 'imagenet'
    base_model = repvgg_A0(True if pretrain == 'imagenet' else False)
